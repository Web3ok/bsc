...
vi.mock('../../src/dex/DEXIntegration', () => ({
  DEXIntegration: vi.fn(() => mockDEXIntegration)
}));
...
describe('批量执行引擎测试', () => {
  ...
  describe('批量交易执行', () => {
    test('应该能够并发执行多个交易', async () => {
      ...
      expect(mockDEXIntegration.executeTrade).toHaveBeenCalledTimes(3);
      
      // 并发执行应该比顺序执行快
      expect(endTime - startTime).toBeLessThan(1000); // 应该在1秒内完成
    });

    test('应该能够顺序执行交易', async () => {
      const trades = [
        {
          type: 'buy' as const,
          tokenIn: '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c',
      ...
      // 顺序执行应该包含延迟时间
      expect(endTime - startTime).toBeGreaterThan(200); // 2 * 100ms 执行时间 + 50ms 延迟
    });

    test('应该能够按时间错开执行交易', async () => {
      ...
      expect(results.every(r => r.success)).toBe(true);
      expect(mockDEXIntegration.executeTrade).toHaveBeenCalledTimes(5);
    });
  });

  describe('错误处理和重试', () => {
    test('应该能够重试失败的交易', async () => {
      const trade = {
        type: 'buy' as const,
        tokenIn: '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c',
        tokenOut: '0x55d398326f99059fF775485246999027B3197955',
      ...
      expect(mockDEXIntegration.executeTrade).toHaveBeenCalledTimes(2);
    });
    ...
    test('应该在达到最大重试次数后失败', async () => {
      ...
      expect(mockDEXIntegration.executeTrade).toHaveBeenCalledTimes(3); // 初始调用 + 2次重试
    });

    test('部分失败的批量交易应该返回混合结果', async () => {
      const trades = [
        {
          type: 'buy' as const,
          tokenIn: '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c',
      ...
      expect(results[1].error).toContain('Insufficient balance');
    });
  });

  describe('批量转账', () => {
    test('应该能够批量转账到多个地址', async () => {
      const transfers = [
        {
      ...
      const mockTransfer = vi.fn().mockResolvedValue(mockTransferResult);
  ...
  describe('性能和限制', () => {
    test('应该限制最大并发数', async () => {
      ...
      expect(maxConcurrent).toBeLessThanOrEqual(3); // 配置的最大并发数
    });

    test('应该能够取消正在执行的批量操作', async () => {
      const trades = Array.from({ length: 5 }, (_, i) => ({
        type: 'buy' as const,
        tokenIn: '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c',
        tokenOut: '0x55d398326f99059fF775485246999027B3197955',
      ...
      // 应该有一些交易被取消
      expect(results.some(r => !r.success && r.error?.includes('cancelled'))).toBe(true);
    });
  });

  describe('监控和报告', () => {
    test('应该提供批量操作的进度报告', async () => {
      const trades = Array.from({ length: 5 }, (_, i) => ({
        type: 'buy' as const,
        tokenIn: '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c',
        tokenOut: '0x55d398326f99059fF775485246999027B3197955',
    ...
    test('应该生成批量操作的执行报告', async () => {
      ...
      expect(report.totalGasUsed).toBe('150000');
