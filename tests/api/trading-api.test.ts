...
describe('交易API端点测试', () => {
  ...
  describe('POST /api/trading/quote', () => {
    test('应该返回有效的交易报价', async () => {
      ...
      expect(response.body.data.tokenOut.amount).toBe('300');
      expect(response.body.data.priceImpact.impact).toBe(0.15);
      expect(response.body.data.slippageAnalysis.recommendedSlippage).toBe(0.5);
      expect(response.body.data.minimumReceived).toBeDefined();
      expect(response.body.data.gasEstimate).toBeDefined();
    });
    ...
    test('应该处理不同的交易金额', async () => {
      ...
      expect(smallResponse.body.data.tokenOut.amount).toBe('30');
      expect(largeResponse.body.data.tokenOut.amount).toBe('3000');
    });
  });
  ...
  describe('POST /api/trading/execute', () => {
    test('应该成功执行交易', async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.data.txHash).toMatch(/^0x[a-fA-F0-9]{40}$/);
      expect(response.body.data.status).toBe('confirmed');
      expect(response.body.data.gasUsed).toBe('142350');
      expect(response.body.data.actualAmountOut).toBe('299.5');
    });

    test('应该拒绝余额不足的交易', async () => {
      const executeRequest = {
        walletAddress: '0x1234567890123456789012345678901234567890',
        tokenIn: '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c',
        tokenOut: '0x55d398326f99059fF775485246999027B3197955',
  ...
  describe('POST /api/trading/batch', () => {
    test('应该成功执行批量交易', async () => {
      const batchRequest = {
        trades: [
          {
            type: 'buy',
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.data.batchId).toMatch(/^batch_\d+$/);
    ...
    test('应该拒绝空的交易数组', async () => {
      ...
      expect(response.body.success).toBe(false);
      expect(response.body.error).toContain('Invalid trades array');
    });

    test('应该处理不同的执行策略', async () => {
  ...
  describe('GET /api/trading/history', () => {
    ...
    test('应该支持钱包地址筛选', async () => {
      ...
      expect(response.body.success).toBe(true);
      expect(response.body.data.every((trade: any) => trade.walletAddress === walletAddress)).toBe(true);
    });

    test('应该包含完整的交易信息', async () => {
      const response = await request(app)
        .get('/api/trading/history')
        .expect(200);
      ...
      expect(trade.tokenIn).toBeDefined();
      expect(trade.tokenOut).toBeDefined();
      expect(trade.walletAddress).toMatch(/^0x[a-fA-F0-9]{40}$/);
      expect(trade.txHash).toMatch(/^0x[a-fA-F0-9]+$/);
      expect(['completed', 'failed', 'pending']).toContain(trade.status);
      expect(trade.timestamp).toBeDefined();
    });
  });

  describe('GET /api/trading/stats', () => {
    test('应该返回交易统计信息', async () => {
      const response = await request(app)
      ...
      expect(typeof stats.successfulTrades).toBe('number');
      ...
      expect(stats.bestPerformingPair).toBeDefined();
      expect(stats.worstPerformingPair).toBeDefined();
    });

    test('统计数据应该保持一致性', async () => {
      const response = await request(app)
        .get('/api/trading/stats')
        .expect(200);

      const stats = response.body.data;
      expect(stats.totalTrades).toBe(stats.successfulTrades + stats.failedTrades);
      expect(Math.abs(stats.successRate - (stats.successfulTrades / stats.totalTrades))).toBeLessThan(0.001);
    });
  });
