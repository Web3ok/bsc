...
describe('钱包管理API端点测试', () => {
  ...
  describe('GET /api/wallets', () => {
    ...
    test('应该支持按组筛选钱包', async () => {
      const response = await request(app)
        .get('/api/wallets?group=test-group')
        .expect(200);

      expect(response.body.success).toBe(true);
      expect(response.body.data.every((w: any) => w.group === 'test-group')).toBe(true);
    });
  });
  ...
  describe('POST /api/wallets/create', () => {
    test('应该创建新钱包', async () => {
      ...
      expect(response.body.data.label).toBe(walletData.label);
      expect(response.body.data.group).toBe(walletData.group);
      expect(response.body.data.status).toBe('active');
    });

    test('应该拒绝没有标签的请求', async () => {
      const response = await request(app)
        .post('/api/wallets/create')
        .send({ group: 'test' })
        .expect(400);
    ...
    test('应该要求私钥当不生成新钱包时', async () => {
      ...
      expect(response.body.success).toBe(false);
      expect(response.body.error).toContain('Private key is required');
    });
  });

  describe('POST /api/wallets/import', () => {
    test('应该导入JSON格式的钱包', async () => {
      const importData = {
        type: 'json',
...
0x2222222222222222222222222222222222222222,CSV Wallet 2,csv-group`;
    ...
    test('应该验证必需参数', async () => {
      ...
      expect(response.body.success).toBe(false);
      expect(response.body.error).toContain('Type and content are required');
    });
  });

  describe('POST /api/wallets/export', () => {
    test('应该导出所有钱包', async () => {
      const response = await request(app)
    ...
    test('应该导出指定的钱包', async () => {
      ...
      expect(response.body.data.wallets[0].address).toBe(walletAddress);
    });

    test('应该处理不存在的钱包地址', async () => {
      const response = await request(app)
        .post('/api/wallets/export')
      ...
      expect(response.body.success).toBe(false);
      expect(response.body.error).toContain('No wallets found to export');
    });
  });

  describe('GET /api/wallets/:address', () => {
    test('应该返回指定的钱包', async () => {
      // 先创建一个钱包
      const createResponse = await request(app)
        .post('/api/wallets/create')
      ...
      expect(response.body.data.label).toBe('Get Test Wallet');
    });

    test('应该处理不存在的钱包', async () => {
      const response = await request(app)
  ...
  describe('PUT /api/wallets/:address', () => {
    test('应该更新钱包信息', async () => {
      // 先创建一个钱包
      const createResponse = await request(app)
        .post('/api/wallets/create')
      ...
      expect(response.body.data.group).toBe(updateData.group);
    });
  ...
  describe('GET /api/wallets/groups', () => {
    test('应该返回钱包组列表', async () => {
      const response = await request(app)
      ...
      if (response.body.data.length > 0) {
        ...
        expect(group.wallets).toBeDefined();
        expect(Array.isArray(group.wallets)).toBe(true);
        expect(group.totalBalance).toBeDefined();
        expect(group.status).toBeDefined();
      }
    });
  });

  describe('POST /api/wallets/groups', () => {
    test('应该创建新的钱包组', async () => {
      const groupData = {
        name: 'new-test-group',
      ...
      expect(response.body.data.strategy).toBe(groupData.strategy);
    });

    test('应该拒绝没有名称的组', async () => {
      const response = await request(app)
        .post('/api/wallets/groups')
  ...
  describe('POST /api/wallets/:address/balance', () => {
    test('应该更新钱包余额', async () => {
      // 先创建一个钱包
      const createResponse = await request(app)
        .post('/api/wallets/create')
      ...
      expect(parseFloat(response.body.data.balance)).toBeGreaterThanOrEqual(0);
    });
