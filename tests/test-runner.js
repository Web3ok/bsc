...
class TestRunner {
    ...
    generateReport(total, passed, failed) {
    ...
    writeReportFile(total, passed, failed, duration) {
        const report = {
        ...
        try {
            (0, fs_1.writeFileSync)('./tests/test-report.json', JSON.stringify(report, null, 2));
        }
        catch (error) {
